...
-- Tạo bảng Xuất kho (Exports) == Chi tiết hóa đơn
...
CREATE FUNCTION CalculateOrderTotal (invoice_id_param INT)
RETURNS DECIMAL(10, 2)
DETERMINISTIC
BEGIN
    DECLARE total DECIMAL(10, 2);

    SELECT SUM(e.quantity * e.unit_price) INTO total
    FROM Exports e
    WHERE e.invoice_id = invoice_id_param;
...
CREATE FUNCTION GetProductPrice (product_id_param INT, invoice_id_param INT)
RETURNS DECIMAL(10, 2)
DETERMINISTIC
BEGIN
DECLARE price DECIMAL(10, 2);
    DECLARE customer_type_id_var INT;

    -- Lấy customer_type_id từ bảng Invoices dựa trên invoice_id
    SELECT c.customer_type_id INTO customer_type_id_var
    FROM Invoices i JOIN Customers c ON i.customer_id = c.customer_id
    WHERE i.invoice_id = invoice_id_param;

    -- Lấy giá từ bảng ProductPrices dựa trên product_id và customer_type_id
    SELECT unit_price INTO price
    FROM ProductPrices
    WHERE product_id = product_id_param AND customer_type_id = customer_type_id_var;
...
-- Thêm dữ liệu mẫu vào bảng Invoices sử dụng function tính tổng
...
-- Thêm dữ liệu mẫu vào bảng Exports sử dụng function
INSERT INTO Exports (export_date, product_id, invoice_id, quantity, unit_price) VALUES
('2023-10-27', 1, 1, 2, GetProductPrice(1, 1)),
('2024-10-27', 2, 2, 5, GetProductPrice(2, 2)),
('2024-12-27', 3, 3, 10, GetProductPrice(3, 3)),
('2025-01-28', 14, 4, 20, GetProductPrice(14, 4)),
('2024-11-01', 16, 5, 10, GetProductPrice(16, 1)),
('2024-11-05', 17, 6, 5, GetProductPrice(17, 2)),
('2024-11-10', 18, 7, 3, GetProductPrice(18, 3)),
('2024-12-01', 8, 8, 10, GetProductPrice(8, 4));


UPDATE Invoices i
SET order_total = CalculateOrderTotal(i.invoice_id)
WHERE i.invoice_id IN (SELECT DISTINCT invoice_id FROM Exports); 
...
BEGIN
    UPDATE Invoices
    SET order_total = CalculateOrderTotal(NEW.invoice_id)
